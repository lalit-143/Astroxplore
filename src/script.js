...
function identifyPlanet(clickedObject) {
  // Logic to identify which planet was clicked based on the clicked object, different offset for camera distance
        ...
        if (clickedObject.userData && clickedObject.userData.name) {
          offset = 10; // Set a small offset for NEOs
          return { name: "Near Earth Object", ...clickedObject.userData };
      }
...
function showPlanetInfo(planet) {
  ...
  name.innerText = planet.name || planet;

  if (planet.name === "Near Earth Object") {
      details.innerText = `Diameter: ${planet.diameter}
      Distance from Earth: ${planet.distance}
      Velocity: ${planet.velocity}
      Potentially Hazardous: ${planet.hazardous}`;
  } else {
      // Existing planet info display code
...
async function fetchNeoData() {
  const API_KEY = "XohyYlbmddUdhOdJT3cRnOWQBgPdgZcz0XR2kb8D";
...
function addNeoToScene(neoData) {
  neoData.forEach((neo, index) => {
      const size = neo.estimated_diameter.meters.estimated_diameter_min / 1000; // Convert to km and scale down
      const distance = neo.close_approach_data[0].miss_distance.kilometers / 1000000; // Convert to million km
      ...
      // Add to raycastTargets for interaction
      raycastTargets.push(neoMesh);

      // Store NEO data for display
      neoMesh.userData = {
          name: neo.name,
          diameter: `${neo.estimated_diameter.meters.estimated_diameter_min.toFixed(2)} - ${neo.estimated_diameter.meters.estimated_diameter_max.toFixed(2)} meters`,
          distance: `${parseFloat(neo.close_approach_data[0].miss_distance.kilometers).toFixed(2)} km`,
...
// After setting up the scene and before the animate function
fetchNeoData().then(neoData => {
  addNeoToScene(neoData);
});